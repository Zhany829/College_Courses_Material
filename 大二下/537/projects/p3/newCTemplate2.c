...
alias_ptr_t alias_add(alias_ptr_t ptr, const char* name, const char* command) {
...
alias_ptr_t execute_alias_command(char* command, alias_ptr_t alias_ptr) {
    ...
    int cnt = split_line(input, alias_line);
    char long_arg[50] = "unalias: Incorrect number of arguments.\n";
    // case: unalias
    if (strncmp(command, "unalias", strlen("unalias")) == 0) {
        ...
        // case: alias + alias-name + replacement name
        // set alias
        char* alias_name;
        char* alias_command;
        // start of alias name
    ...
    if (incorrect_usage) {
        char alias_long_error[105] = "alias: Too dangerous to alias that.\n";
        write(STDERR_FILENO, alias_long_error,strlen(alias_long_error));
    }
    return alias_ptr;
}
void print_error() {
