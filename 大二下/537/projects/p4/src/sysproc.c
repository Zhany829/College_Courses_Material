...
sys_sleep(void)
{
  ...
  acquire(&tickslock);
  ticks0 = ticks;
  myproc()->cur_sleep_ticks = n; // com time
  myproc()->sleepticks += n; // total sleep time
  myproc()->wakeupat = n + ticks0;  // save wake up time 
  myproc()->curticks = 0; // remove cur ticks -- sleep
