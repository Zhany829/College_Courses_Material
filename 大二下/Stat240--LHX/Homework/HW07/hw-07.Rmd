...
- The facility Las Campanas Observatory is fifth on the list and it appears 29 times.
...
- regular: `^2MASS J04414489\+2301513$`
- string:  `^2MASS J04414489\\+2301513$`
...
planets %>%
  ...
  mutate(planet = str_replace_all(str_c("/", planet, "/"), " ", "_"),
         star = str_replace_all(str_c("/", star, "/"), " ", "_"))
```

- There are 27 such planets do not follow the naming convention.
...
- There are 4246 cases is the suffix a single lower case letter
...
```{r}
planets %>%
  mutate(update_format = case_when(str_detect(update, " ") ~ "datetime", !str_detect(update, " ") ~ "date")) %>%
  group_by(update_format) %>%
  summarise(count = n())
```

- There are 1376 date format and 2900 datetime formate
...
- P(X = 3) = 1 - P(X = 1) - P(X = 2) - P(X = 5) = 1 - 0.1 - 0.4 - 0.2 = 0.3
...
```{r}
prob8 <- replace_na(prob7, list(p = 0.3))
# expected value (mean)
sum(prob8[["x"]] * prob8[["p"]])

# variance of the distribution
sum((prob8[["x"]] - 2.8)^2 * prob8[["p"]])
```
...
```{r} 
ggplot(prob8, mapping = aes(x = x, y = p)) +
  geom_point() +
...
```{r}
prob8 %>%
  add_row(x = 0, p = 0, .before = 1) %>%
  add_row(x = 6, p = 0) %>%
  ggplot(mapping = aes(x = x, y = cumsum(p))) +
  geom_step() +
  xlim(0, 6)
```
