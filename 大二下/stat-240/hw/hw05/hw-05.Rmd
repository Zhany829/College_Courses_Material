...
Answer: There are 29450 rows and 24 columns in the data set.
...
Answer:
For 2016, 2017 and 2018, Tuesday has the lowest average number of passengers.
For 2019, Thurday has the lowest average number of passengers.
```{r}
ord_3 <- ord%>%
  filter(year != 2020)%>%
  group_by(wday,year,date)%>%
  summarise(passengers = sum(passengers))%>%
  group_by(year,wday)%>%
  summarize(avg = mean(passengers))


ggplot(data=ord_3, aes(x=wday,y=avg,fill=year)) +
  facet_wrap(~year)+
  geom_col()+
...
Answer:
The amount of passengers usually increase from February to July and gradually decrease from August to December. However, the amount of passengers in 2020 months is very small except the first two months
```{r}
ord_4 <- ord %>%
  group_by(month, year) %>%
...
Answer:
The average waiting time of US passengers is proportional to that of Non-US passengers

What are the primary differences between US and Non-US passengers, and are the patterns different for different seasons?

Answer:
Domestic passengers usually have less average waiting time than international passengers and
it seems like summer has the longest average waiting time, while winter has the shortest.

```{r}
ord_season<-ord %>%
  mutate(m=month(date))%>%
  mutate(season = case_when(m==3 | m==4| m==5 ~ "Spring",
                            m==6 | m==7 | m==8 ~ "Summer",
                            m==9 | m==10 | m==1 ~"Fall",
                            TRUE ~ "Winter")) 

ggplot(data = ord_season, mapping = aes(x = us_avg_wait,y = non_us_avg_wait, color = season))+
  geom_point() +
  geom_abline(slope = 1) +
  geom_smooth(method = "lm") +
  facet_wrap(~year) +
  labs(x = "US passengers average wait time", y = "Non-US passengers average wait time")
